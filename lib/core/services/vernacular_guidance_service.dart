...
class VernacularGuidanceService {
  ...
  static VernacularGuidance _generateHindi(
    ...
    if (band == RiskBand.red) {
      ...
      if (s.temperatureC >= 39.5) {
        reasons.add(
          'तापमान ${s.temperatureC.toStringAsFixed(1)}°C गंभीर रूप से अधिक है',
        );
      }
      ...
      return VernacularGuidance(
        ...
        spokenText:
            'सतर्कता: $who में $reasonText। कृपया तुरंत नजदीकी सामुदायिक स्वास्थ्य केंद्र या अस्पताल में रेफर करें।',
        immediateAction:
            'मरीज को स्थिर रखें। यदि सांस लेने में तकलीफ हो तो सीधा बैठाएं। तुरंत 108 एम्बुलेंस या वाहन की व्यवस्था करें।',
        dangerSigns: const [
    ...
    if (band == RiskBand.yellow) {
      return VernacularGuidance(
        ...
        spokenText:
            'ध्यान दें: $who की जांच में कुछ आंकड़े असामान्य पाए गए हैं (स्कोर ${a.score}/100)। चौबीस घंटे के भीतर नजदीकी स्वास्थ्य केंद्र में डॉक्टर से परामर्श लें।',
        immediateAction:
            'रोगी को आराम कराएं। पर्याप्त पानी और तरल पदार्थ दें। लक्षणों के बिगड़ने पर तुरंत अस्पताल ले जाएं।',
        dangerSigns: const [
    ...
    return VernacularGuidance(
      ...
      spokenText:
          '$who के सभी प्राथमिक महत्वपूर्ण लक्षण (हृदय गति, ऑक्सीजन, तापमान) सुरक्षित सीमा में हैं।',
      immediateAction:
          'नियमित स्वास्थ्य देखभाल जारी रखें। यदि नए लक्षण दिखाई दें तो दोबारा जांच करें।',
  ...
  static VernacularGuidance _generateBengali(
    ...
    if (band == RiskBand.red) {
      ...
      if (s.temperatureC >= 39.5) {
        reasons.add(
          'শরীরের তাপমাত্রা ${s.temperatureC.toStringAsFixed(1)}°C অতি উচ্চ',
        );
      }
      if (s.heartRateBpm > 120 || (s.heartRateBpm > 0 && s.heartRateBpm < 45)) {
      ...
      final reasonText = reasons.isNotEmpty
          ? reasons.join(', ')
          : 'জরুরি বিপদের লক্ষণ পাওয়া গেছে';

      return VernacularGuidance(
        languageCode: 'bn',
        languageName: 'বাংলা',
        headline:
            'জরুরি সতর্কতা: অবিলম্বে হাসপাতালে স্থানান্তর করুন (লাল বিভাগ)',
        spokenText:
            'সতর্কতা: $who-এর $reasonText। অবিলম্বে নিকটবর্তী স্বাস্থ্যকেন্দ্রে বা হাসপাতালে নিয়ে যান।',
        immediateAction:
            'রোগীকে সোজা বসিয়ে রাখুন। দ্রুত অ্যাম্বুলেন্স বা যানবাহনের ব্যবস্থা করুন।',
        dangerSigns: const [
    ...
    if (band == RiskBand.yellow) {
      return VernacularGuidance(
        ...
        spokenText:
            'দৃষ্টি আকর্ষণ: $who-এর কিছু পরিমাপ স্বাভাবিকের চেয়ে বেশি (স্কোর ${a.score}/১০০)। আগামী ২৪ ঘণ্টার মধ্যে ডাক্তারের পরামর্শ নিন।',
        immediateAction:
            'রোগীকে বিশ্রাম দিন এবং প্রচুর পরিমাণে তরল খাবার ও জল পান করতে দিন।',
        dangerSigns: const [
          'জ্বর ক্রমাগত বাড়লে',
          'বারবার বমি হলে',
    ...
    return VernacularGuidance(
      ...
      spokenText:
          '$who-এর প্রাথমিক স্বাস্থ্য মাপকাঠি (অক্সিজেন, হৃদস্পন্দন, তাপমাত্রা) স্বাভাবিক সীমার মধ্যে রয়েছে।',
      immediateAction:
          'নিয়মিত স্বাস্থ্যের যত্ন নিন। নতুন উপসর্গ দেখা দিলে পুনরায় স্ক্রিনিং করুন।',
  ...
  static VernacularGuidance _generateEnglish(
    ...
    if (band == RiskBand.red) {
      ...
      if (s.estimatedGlucose > 0 && s.estimatedGlucose < 70) {
        reasons.add(
          'Blood glucose (${s.estimatedGlucose} mg/dL) indicates hypoglycemia',
        );
      } else if (s.estimatedGlucose >= 250) {
        reasons.add(
          'Blood glucose (${s.estimatedGlucose} mg/dL) is critically elevated',
        );
      }
      if (s.temperatureC >= 39.5) {
      ...
      if (s.heartRateBpm > 120 || (s.heartRateBpm > 0 && s.heartRateBpm < 45)) {
        reasons.add(
          'Heart rate (${s.heartRateBpm} BPM) is out of safety range',
        );
      }

      final reasonText = reasons.isNotEmpty
          ? reasons.join(', ')
          : 'Critical red flag vitals detected';
      ...
      return VernacularGuidance(
        ...
        spokenText:
            'Warning: $who exhibits danger signs: $reasonText. Immediate transport to Community Health Centre or Hospital is required.',
        immediateAction:
            'Keep patient seated upright if breathless. Arrange immediate emergency medical transport. Do not leave patient unattended.',
        dangerSigns: const [
    ...
    if (band == RiskBand.yellow) {
      return VernacularGuidance(
        ...
        spokenText:
            'Notice: $who has abnormal screening findings with a risk score of ${a.score}/100. Schedule a medical examination at the local clinic within 24 hours.',
        immediateAction:
            'Ensure patient rests and remains well-hydrated. Re-screen immediately if danger signs develop.',
        dangerSigns: const [
    ...
    return VernacularGuidance(
      ...
      spokenText:
          'All primary vitals for $who are within standard screening ranges.',
      immediateAction:
          'Maintain routine health monitoring. Re-screen if symptoms arise.',
