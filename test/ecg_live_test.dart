...
Future<TestHarness> _harness(
  ...
  final harness = await TestHarness.create(
    overrides: [
      ...
      if (fakeStore)
        waveformStoreProvider.overrideWith(
          (ref) => _RecordingWaveformStore(
...
void main() {
  group('provenance', () {
    testWidgets(
      'no board streaming reads DEMO, and says the trace is generated',
      (tester) async {
        ...
        await tester.pumpWidget(
          harness.wrapRouted(() => const EcgLiveScreen()),
        );
        await tester.pump(const Duration(milliseconds: 400));
        ...
        expect(
          ...
          findsOneWidget,
          reason:
              'a synthetic trace has to say so on the strip, not only in a '
              'three-letter badge',
        );
        expect(
    ...
    testWidgets(
      'losing the board mid-session switches the strip to generated',
      (tester) async {
        await tester.useSmallPhone();
        final service = _FakeBleService(link: _streaming);
        final harness = await _harness(service);
  ...
  group('measurements', () {
    ...
    testWidgets('electrodes off the skin is stated above the strip', (
      ...
      expect(
        find.textContaining('electrodes are off the skin'),
        findsOneWidget,
      );
  ...
  group('recording', () {
    ...
    testWidgets('samples that arrived before recording are not counted in', (
      ...
      expect(
        harness.container.read(screeningDraftProvider).ecgSamples.length,
        250,
      );
    });
    ...
    testWidgets('lead switching on the generated trace clears the strip', (
      ...
      expect(
        find.textContaining('Starting the generated trace'),
        findsOneWidget,
      );
      for (final label in _readings) {
        expect(_reading(tester, label), '—');
  ...
  group('saving', () {
    ...
    testWidgets('refuses while the recording is still running', (tester) async {
      ...
      expect(
        find.textContaining('Stop the recording before saving'),
        findsOneWidget,
      );
      expect(
        harness.container.read(screeningDraftProvider).ecgSamples,
    ...
    testWidgets('a measured strip lands in the draft at the real rate', (
      ...
      expect(
        find.textContaining('3.0 s (750 samples at 250 Hz)'),
        findsOneWidget,
      );
    ...
    testWidgets('a generated strip needs confirming and forces demo', (
      ...
      expect(
        harness.container.read(screeningDraftProvider).isDemoDevice,
        isFalse,
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'a generated waveform on a real device still makes the whole '
            'screening a demo',
      );
    ...
    testWidgets(
      'a measured strip on a saved screening is written to the blob',
      (tester) async {
        await tester.useSmallPhone();
        final service = _FakeBleService(link: _streaming);
        final harness = await _harness(
        ...
        final store =
            harness.container.read(waveformStoreProvider)
                as _RecordingWaveformStore;
