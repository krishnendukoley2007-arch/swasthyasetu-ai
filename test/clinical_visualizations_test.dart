...
void main() {
  group('Clarke Error Grid Analysis (EGA)', () {
    test(
      'evaluates clinical zones accurately according to ISO 15197 / Clarke 1987',
      () {
    ...
    testWidgets('renders ClarkeErrorGridWidget without layout overflow', (
      tester,
    ) async {
  ...
  group('Poincaré Plot (RR Interval HRV)', () {
    testWidgets(
      'renders PoincarePlotWidget with telemetry tiles and SD1/SD2 metrics',
      (tester) async {
  ...
  group('Vascular Elasticity & Pulse Transit Time Speedometer', () {
    test('computes pulse wave velocity (PWV) accurately from PTT', () {
      ...
      expect(
        stiffGauge.stiffnessClassification,
        'High Arterial Stiffness (Rigid)',
      );
    });

    testWidgets(
      'renders VascularElasticityGauge with numeric readout and explainer',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(
        ...
        expect(
          find.text('Vascular Elasticity & PTT Speedometer'),
          findsOneWidget,
        );
  ...
  group('Dual Waveform Sweep Monitor', () {
    testWidgets('renders ECG + Pleth PPG sweep monitor with telemetry HUD', (
      tester,
    ) async {
      await tester.pumpWidget(
        const MaterialApp(
  ...
  group('Ayushman Bharat ABHA QR Card', () {
    testWidgets(
      'renders offline ABHA card with valid FHIR R4 JSON QR payload',
      (tester) async {
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: SingleChildScrollView(
                child: AbhaQrCard(patient: patient, screening: screening),
              ),
            ),
          ),
        );
  ...
  group('High-Fidelity PDF Medical Report (Ayushman Bharat / CHC)', () {
    test(
      'generates valid multi-section PDF document with non-empty bytes',
      () async {
        final patient = Patient(
          id: 'P-202',
          name: 'Sunita Sharma',
