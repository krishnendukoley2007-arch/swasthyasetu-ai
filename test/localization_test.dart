...
void main() {
  group('supported locales', () {
    ...
    test('hi and bn are not silently serving the English strings', () async {
      ...
      for (final locale in [const Locale('hi'), const Locale('bn')]) {
        ...
        expect(
          identical,
          lessThan(_criticalKeys.length ~/ 4),
          reason:
              '${locale.languageCode} matches English on $identical of '
  ...
  group('triage vocabulary', () {
    test(
      'every band, sync state and escalation level has a word in each locale',
      () async {
        for (final locale in _locales) {
          final l10n = await _load(locale);
    ...
    test(
      'an unknown code falls through to a real word, never a blank',
      () async {
    ...
    test(
      'RiskStyle keeps its colours and icons when it is localized',
      () async {
    ...
    test(
      'symptom durations translate for display but not for storage',
      () async {
        final l10n = await _load(const Locale('bn'));
