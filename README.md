...
![Tests](https://img.shields.io/badge/tests-430%20passing-success?style=for-the-badge)
...
flutter test                       # ✅ 430 tests — all should pass!
...
test/               🧪 430 tests (overflow @ 2.0x font, offline-map honesty, etc.)
firmware/           🔌 ESP32 Arduino sketch (SSAI_SENSE_01) — ECG, touch, OLED, battery
tools/              💻 ecg_dashboard.html — laptop Web-Bluetooth live ECG viewer
assets/
...
The reference sensor-node firmware lives in [`firmware/SSAI_SENSE_01/`](firmware/SSAI_SENSE_01/SSAI_SENSE_01.ino).

1. Install **Arduino IDE** → add ESP32 board support (esp32 by Espressif, 3.x)
2. Install libraries: **Adafruit SSD1306**, **Adafruit GFX**
3. Open `SSAI_SENSE_01.ino` → board: *DOIT ESP32 DEVKIT V1* → **Upload**
4. On boot: OLED shows version → tap the touch pad → live HR/ECG/BP/battery display, BLE advertises as the app expects

Default pin map: AD8232 out → GPIO 34, LO+/LO− → GPIO 32/33, touch → GPIO 4 (active-LOW), OLED SDA/SCL → 21/22 (0x3C), battery divider (2×100k) → GPIO 35, LED → GPIO 2.

### 💻 Laptop Demo Dashboard

No phone handy? Open [`tools/ecg_dashboard.html`](tools/ecg_dashboard.html) in **Chrome/Edge** → click Connect → pick the ESP32 over Web Bluetooth → live ECG strip + vitals in the browser. Perfect for demos and quick hardware sanity checks.
...
[🐛 Report Bug](../../issues) • [💡 Request Feature](../../issues/new) • [📋 Changelog](CHANGELOG.md) • [🔒 Security](SECURITY.md)
