...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  double _dailyStrain = 0.0;
  final double _targetStrain = 14.5;
  ...
  void initState() {
    ...
    _motionSub = _bleService.motionStream.listen((motion) {
      if (mounted) {
        setState(() {
          _dailySteps = motion.stepCount;
          _currentCalories =
              (_dailySteps * 0.045 + (motion.motionIntensity * 0.8)).round();
          if (_dailySteps > 0) {
            _dailyStrain = (21.0 * (1.0 - math.exp(-0.00025 * _dailySteps)))
                .clamp(0.0, 21.0);
          }
        });
      }
  ...
  Widget build(BuildContext context) {
    final currentZone = HeartRateZone.getZoneForBpm(_currentBpm ?? 70);
          ...
          children: [
            ...
            StreamBuilder<BleConnectionState>(
              ...
              builder: (context, snapshot) {
                ...
                final isReconnecting = state == BleConnectionState.reconnecting;
                final isScanning = state == BleConnectionState.scanning;

                Color bannerColor = Colors.redAccent;
                if (isConnected) {
                  bannerColor = FitnessTheme.neonLime;
                } else if (isSimulated || isReconnecting || isScanning) {
                  bannerColor = Colors.amber;
                }

                String titleText = 'No Wearable Connected';
                if (isConnected) {
                  titleText =
                      'FitPulse Band: ${_bleService.connectedDeviceName ?? "Connected"}';
                } else if (isSimulated) {
                  titleText = 'Sample / Demo Simulation Active';
                } else if (isReconnecting) {
                  titleText = 'Reconnecting to FitPulse Band...';
                } else if (isScanning) {
                  titleText = 'Searching for FitPulse Wearable...';
                }
              ...
              children: [
                Expanded(
                  child: _buildMetricTile(
                    title: 'CALORIES',
                    value: _dailySteps > 0 ? '$_currentCalories' : '--',
                    unit: 'kcal',
                ...
                Expanded(
                  child: _buildMetricTile(
                    title: 'STEPS',
                    value: _dailySteps > 0 ? '$_dailySteps' : '--',
                    unit: 'steps',
